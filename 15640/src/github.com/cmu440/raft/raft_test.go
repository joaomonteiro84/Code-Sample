...
func TestReElection2C(t *testing.T) {
	fmt.Printf("==================== 3 SERVERS ====================\n")
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()

	fmt.Printf("Test (2C): Mimic the Re-election hidden test in gradescope\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()
	...
	fmt.Printf("leader 2 = %d, other leader = %d\n", leader2, cfg.checkOneLeader())
	...
	fmt.Printf("check if there is a leader\n")
	leader4 := cfg.checkOneLeader()
	fmt.Printf("leader 4 = %d\n", leader4)
...
func TestLeaderConsistency(t *testing.T) {
	fmt.Printf("==================== 3 SERVERS ====================\n")
	servers := 3
	...
	fmt.Printf("Test (2D): Mimic the leader consistency hidden test in gradescope\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()
	fmt.Printf("leader1 = %d\n", leader1)
	...
	fmt.Printf("reconnecting old peer\n")
	cfg.connect(leader1)

	fmt.Printf("checking if there is a leader\n")
	leader3 := cfg.checkOneLeader()
	fmt.Printf("leader 3 = %d, other leader = %d\n", leader3, cfg.checkOneLeader())

	fmt.Printf("reconnecting old leader\n")
	cfg.connect(leader2)

	fmt.Println("checking current leader")
	fmt.Printf("leader  = %d\n", cfg.checkLeaderInPartition(m))

	fmt.Printf("======================= END =======================\n\n")
}

func TestSmallPartitionNoConsensus1(t *testing.T) {
	...
	fmt.Printf("Checking for previous leader in partition 2\n")
	fmt.Printf("leader in p2 = %d, leader before partition =%d\n", cfg.checkLeaderInPartition(p2), leader1)

	cfg.connect_partition(p2)
...
func TestSmallPartitionNoConsensus2(t *testing.T) {
	fmt.Printf("==================== 5 SERVERS ====================\n")
	servers := 5
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
...
func TestFailAgree2B(t *testing.T) {
	...
	//fmt.Printf("Peer disconnected: %d\n", (leader+1)%servers)
...
func TestRejoinHidden2B(t *testing.T) {
	...
	i1, t1, l1 := cfg.rafts[leader].PutCommand(20)
	i2, t2, l2 := cfg.rafts[leader].PutCommand(30)
	i3, t3, l3 := cfg.rafts[leader].PutCommand(40)

	fmt.Printf("old leader commit index: %d \n", cfg.rafts[leader].commitIndex)

	fmt.Println(i1, t1, l1, i2, t2, l2, i3, t3, l3)

	fmt.Printf("Checking agreement for new leader\n")
	newleader := cfg.checkOneLeader()
	fmt.Printf("leader 2: %d\n", newleader)

	cfg.one(25, servers-1)
...
func TestBackupHidden2B(t *testing.T) {
	...
	p2[leader] = emptyStruct
	p2[(leader+1)%servers] = emptyStruct

	fmt.Printf("leader:%d sidekick:%d\n", leader, (leader+1)%servers)

	cfg.disconnect_partition(p2)

	fmt.Printf("Submitting lots of commands to leader + follower partition which should not commit\n")

	for i := 0; i < 50; i++ {
		cfg.rafts[leader].PutCommand(i)
	}
	...
	for i := 50; i < 90; i++ {
		cfg.rafts[newleader].PutCommand(i)
	}

	fmt.Println("Disconnecting a follower from the current partition")
	follower := (newleader + 1) % servers
	follower2 := (newleader + 2) % servers

	if follower == leader {
		follower = (leader + 2) % servers
		follower2 = (leader + 3) % servers
	} else if follower2 == leader {
		follower2 = (leader + 2) % servers
	}

	fmt.Printf("new leader: %d sidekick1: %d sidekick2: %d\n", newleader, follower, follower2)

	fmt.Printf("follower being disconnected: %d\n", follower)

	cfg.disconnect(follower)

	fmt.Printf("Submitting lots of commands to the new partition which should not commit\n")
	...
	newnewleader := cfg.checkOneLeader()

	fmt.Println("Submitting lots of commands to the new partition which should commit")
	for i := 1700; i < 1780; i++ {
		cfg.rafts[newnewleader].PutCommand(i)
	}
...
func TestCountHidden2B(t *testing.T) {
	fmt.Printf("==================== 3 SERVERS ====================\n")
	servers := 3
	cfg := make_config(t, servers, false)
	defer cfg.cleanup()
	...
	for i := 0; i < 10; i++ {
		cfg.rafts[leader].PutCommand(i)
		rpccalls += (cfg.rpcCount(leader) + cfg.rpcCount((leader+1)%3) + cfg.rpcCount((leader+2)%3))
	}

	fmt.Printf("number rpc calls:%d\n", rpccalls)
