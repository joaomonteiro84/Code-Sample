...
def select_child(config, node, min_max_stats):
    ...
    for a, c in node.children.items():
        child_score = ucb_score(config, node, c, min_max_stats)
...
def expand_root(node, actions, network, current_state):
    ...
    value, reward, policy_logits, hidden_representation = network.initial_inference(np.expand_dims(current_state, axis=0))
    node.hidden_representation = hidden_representation[0].numpy()

    node.reward = reward

    # Extract softmax policy and set node.policy
    node.policy = network._softmax(policy_logits)[0].numpy()
...
def expand_node(node, actions, network, parent_state, parent_action):
    ...
    # get hidden state representation
    value, reward, policy_logits, hidden_representation = network.recurrent_inference(np.expand_dims(parent_state, axis=0), parent_action)
    node.hidden_representation = hidden_representation[0].numpy()
...
def backpropagate(path, value, discount, min_max_stats):
    ...
    for node in reversed(path):
        # YOUR CODE HERE
        node.visit_count += 1
        node.value_sum += trajectory_value
        trajectory_value = node.reward + discount*trajectory_value
...
def softmax_sample(visit_counts, temperature):
    ...
    v_counts = np.array([x[0] for x in visit_counts])   


   # YOUR CODE HERE
    if temperature == 0:
        return actions[np.argmax(v_counts)]
    else:
        power_visit_count = v_counts**(1/temperature)
        sample_prob = power_visit_count/np.sum(power_visit_count)

        return np.random.choice(actions, p=sample_prob)
