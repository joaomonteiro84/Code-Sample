...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        #network.
        pred_hidden_representation, pred_value, pred_policy_logits = network.initial_model.__call__(np.array(state_batch))
        ...
        init_value_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, pred_value))
        init_policy_loss =tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_policy_batch, pred_policy_logits))
        total_value_loss += init_value_loss.numpy()
        total_policy_loss += init_policy_loss.numpy()
        
        loss =  0.25 * init_value_loss + init_policy_loss
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            conditioned_hidden_state = tf.concat((pred_hidden_representation, tf.one_hot(actions_batch, network.action_size)), axis=1)

            pred_hidden_representation, pred_reward, pred_value, pred_policy_logits = network.recurrent_model.__call__(conditioned_hidden_state)
            ...
            # Remember to scale value loss!
            # Add to total losses
            recurrent_value_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, pred_value))
            recurrent_policy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_policy_batch, pred_policy_logits))
            recurrent_reward_loss = tf.keras.losses.MeanSquaredError()(pred_reward,target_reward_batch)

            total_value_loss += recurrent_value_loss.numpy()
            total_policy_loss += recurrent_policy_loss.numpy()
            total_reward_loss += recurrent_reward_loss.numpy()      

            # YOUR CODE HERE: Half the gradient of the representation
            pred_hidden_representation = scale_gradient(pred_hidden_representation, 0.5)

            # YOUR CODE HERE: Sum the losses, scale gradient of the loss, add to overall loss
            step_loss =  0.25 * recurrent_value_loss + recurrent_policy_loss + recurrent_reward_loss
            step_loss =  scale_gradient(step_loss, 1/config.num_unroll_steps) 
            loss += step_loss
        
        print("total value loss: ", total_value_loss)
        print("total policy loss: ", total_policy_loss)
        print("total reward loss: ", total_reward_loss)
