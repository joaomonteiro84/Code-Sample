    ...
    if k in ['Wlayer1', 'Woutput']:            
        for i in range(params[k].shape[0]):
            for j in range(params[k].shape[1]):   
                ...
                if k == 'Wlayer1':
                    params['grad_Wlayer1'][i,j] = num_gradient
                else:
                    params['grad_Woutput'][i,j] = num_gradient
                    
        
        if k in ['blayer1', 'boutput']:           
            
            for i in range(params[k].shape[0]):
                value_backup = params[k][i]
                ...
                if k == 'blayer1':
                    params['grad_blayer1'][i] = num_gradient
                else:
                    params['grad_boutput'][i] = num_gradient        
...
for k in params.keys():
    if 'grad_' in k:
        # relative error
        den = np.maximum(np.abs(params[k]),np.abs(params_orig[k]))
